...
const [showCancelModal, setShowCancelModal] = useState(false);
  ...
  switch (status.toLowerCase()) {
    ...
    case "order canceled": // in case you have both spellings
...
const handleCancelOrder = async () => {
  ...
  {},
  { headers: { Authorization: `Bearer ${token}` } } // works with new middleware
);
  ...
  <DialogContent>
    ...
    <DialogFooter>
      ...
      <Button className="bg-red-600" onClick={handleCancelOrder}>Yes, Cancel</Button>
    </DialogFooter>
  </DialogContent>
</Dialog>
                    ...
                    {reviewOrder.purchasedProducts.map((p) => {
                      ...
                      return (
                        <div key={p._id} className="border rounded-xl p-4 bg-white space-y-3">
                          ...
                          {hasReview ? (
                            <>
                              <StarRating value={p.review?.rating ?? 0} onChange={() => {}} disabled />
                              <p className="text-sm italic">"{p.review?.message}"</p>
                             <p>{p.review?.reviewedAt && new Date(p.review.reviewedAt).toLocaleDateString()}</p>
